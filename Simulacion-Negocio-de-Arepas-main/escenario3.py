...
n_arepa1 = round(CLIENTES * 0.40)
n_arepa2 = round(CLIENTES * 0.45)
...
print ("total = ", n_arepa1+ n_arepa2+n_no_compro)       
...
# 2 para arepa 2 que es la arepa de bebé
# 3 para arepa 3 que en la arepa de morcilla
...
def arepas(x):
    ...
    if (x == 0) : 
        t_no_compro = round(10 * (1 - math.exp((- 1.5 * (random.uniform(0, 1))))))
        return {'compro': "no compro", 'tiempo_atencion': t_no_compro, 'arepa': x}
        


    elif (x == 1):
        t_arepa1 = round(random.uniform(5, 10))
        return {'compro': "compro arepa con todo ", 'tiempo_atencion': t_arepa1, 'arepa': x}
        

    elif (x == 2) :
        t_arepa2 = round(random.uniform(3, 7))
        return {'compro': "compro arepa de bebé ", 'tiempo_atencion': t_arepa2, 'arepa': x}
...
print ("UTILIDAD TOTAL  = ",  n_arepa1*750  + n_arepa2*550  )
...
print ("UTILIDAD TOTAL QUE NO SE GANO = ", (a1- n_arepa1)*750  + (a2- n_arepa2)*550  )
